...
if [ -s $outDir/domainLists.txt ]; then
	...
	#amass enum -silent -df $outDir/domainLists.txt -config $AmassConfig -o $outDir/AmassOut.txt #uncomment out this line and comment the line above if you have config file.
	echo -e "${BGreen}Amass scanning......${BRed}[DONE]"

	subfinder -silent -dL $outDir/domainLists.txt -o $outDir/SubfinderOut.txt &>/dev/null
	#subfinder  -silent -dL $outDir/domainLists.txt -config $SubfinderConfig -o $outDir/SubfinderOut.txt &>/dev/null #uncomment out this line and comment line above if you have config file.
	...
	echo -e "${BBlue}No extract domain from Nmap..Skipping Subdomain enumeration..${BRed}[SKIP]"
fi
